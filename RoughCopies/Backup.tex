...
\altaffiliation{Department of Mathematics, York University, Toronto, ON, Canada.  M3J 1P3}
...
\author{Ozzy Mermut}
\affiliation{Department of Physics and Astronomy, York University, Toronto, ON, Canada. M3J 1P3}
%don't know how to get the star exclusively and don't know if it matters
...
Backup ( dont edit)  %demonstration document.  An abstract is only allowed for certain
...
%not required, because the effect of electrostatic forces are negligible, as stated in the experimental section.

\begin{figure}[H] %exactly here, close to h!
\includegraphics[scale=0.5]{fig7.png}
\caption{}
\label{figure 7a}
\end{figure}

%In the below model we will be sampling all possible configurations of long chain polymers ($PAA_{450k}$)  and short chain polymer ($PAA_{2k}$) in a scenario where the surface of the colloid ($Si0_2$ with PAH Coating) will be almost completely covered by a combination of long and short polymers. The resultant equation will output a value that is proportional to the probability of that combination in the experiment.
...
%double check the notation matches up
...
What was done, in the next section was showing that because every scenario for $j\in[0,\frac{a}{n\cdot W}]$ has been considered from an initial ($j = 0$ ), and consecutive ($j,j+1,j+2,...,j*$ ) ratio values ($\frac{\mathcal{E}_{j+1}}{\mathcal{E}_{j}}$) that are all output values larger than one, and all consecutive values for $j\in[j*,\frac{a}{n \cdot W}]$ are outputting ratio values less than one. Combining the above two statements gives us that the $\mathcal{E}_j$ reaches a maximal point for $j=j^*$ , which is quite close to 100\% surface coverage by Long Chains Event ( $j^*\approx \frac{a}{n\cdot W}$ ) %Recall why we came to that conclusion that it is close to a/nW, either in text below or from your notes, the whole density arguement, "1-j/a" 

The consequence of this is precisely that one can show that the probability of the $j^{th}$ event is approaching a maximal value (${\mathcal{E}_{j}} \to MAX$),where that the ratio is approaching one ($\frac{\mathcal{E}_{j+1}}{\mathcal{E}_{j}}\to 1$) for $j\in[0,j^*]$, and zero otherwise. % given that we see this behaviour in our result it confirm this, may not be able to show the less than one side due to the size of the colloids, orders of magnitude of the arguments inputted into the approximation
...
%maybe even show where it is cutoff in the original image, due to the constraints of volume available on the sphere, I have not been able to provide a volume that has it passing one though
