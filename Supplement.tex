...
We assume that there are $n_L$ ``long'' chains, each
...
$S_{latt}/\leng_S$, which we also call $a$. For 
...
\\
 ...
 \\
 ($ii$) the number of $\leng_S$-mers adsorbed on the surface is $n(a-j)$; and 
...
% \end{verse}  
\\
We write $\mathcal{E}_j$ for the set of all configurations that have exactly $j$ 
$\leng_L$-mers adsorbed 
...
\begin{eqnarray}
   ...
   \nonumber    \\
   & &    \hspace{22mm}
     \prod_{i=2}^{nW-1}(q-1)\left(\frac{ a\leng_L-(k-1)\leng_L-i}{a\leng_L}\right)  
      \nonumber \\
...
\begin{equation}
    \label{eq.Yratio2}
       ...
       \left(  \frac{V_{latt}A_3\leng_S^{\gamma_3-1}(q-1)^2}{aq\leng_S} \,
...
\]
We observe that $K\gg n$; indeed,  $V_{latt}/a\leng_S n \,=\,V_{latt}/A_{latt}\gg 1$.  
