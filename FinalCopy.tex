...
and since every configuration in $\mathcal{E}$ has the number of such contacts
...
Specifically, denoting the number of configurations in a set $\mathcal{A}$ by $|\mathcal{A}|$, we have that
the probability that $j$ long polymers are adsorbed is $\frac{|\mathcal{E}_j|}{|\mathcal{E}|}$
...
To make the total mass of the two kinds of polymers in our system be the same, we can assume that $n_S \,=\, n\cdot n_L$.  However, we do not really use this relation; it is enough that $n_s$ and $n\cdot n_L$
...
$h(j)$ is 
less than 1. We shall show show that $j^*$ is the most probable value of $j$ 
%added this parenthesis as Ozzy notated the significance of j*. May be repetitive
(the number of long chains adsorbed onto the surface).
...
\end{equation}
The above equation can be interpreted as follows.  The left hand side, $1-\frac{j^*}{a}$, is the fraction of the colloid sphere's surface that is covered by short monomers.  The ratio $n_S\leng_S/V_{latt}$ gives
