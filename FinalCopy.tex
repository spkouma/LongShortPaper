...
\begin{abstract}
%Understanding the adsorption behavior of charged self-assembling macromolecules onto surfaces is important in various applications governed by biointerfacial phenomena, soft matter films fabrication, and biomedical engineering of tissue scaffolds.  
We study here the role of polyelectrolyte chain length, that is number of mers, in the competitive adsorption of a simple model polyanion, poly(acrylic acid), onto 85nm spherical silica particles capped with a model polycation, poly(allylamine hydrochloride).  
%Theoretical calculations for neutral polymers show that there is preferential adsorption that is dependant on the chain length, number of mers, of the polymer.  
Performing fluorescence spectroscopy experiments, we measured chain-length dependence of polyelectrolyte adsorption onto an oppositely charged polyelectrolyte covering the spherical silica nanoparticle surface under dilute aqueous solution conditions ($10^{-3}$ g/L).  Preferential adsorption was determined by comparing the characteristic fluorescence intensities  of the  two  fluorophore-labeled  and narrowly disperse
%(NMA-PAA\textsubscript{450K},  and Dan-PAA\textsubscript{2K}) 
polyacrylic acid samples (NMA-PAA\textsubscript{450K}  and Dan-PAA\textsubscript{2K})
of 450K- and 2K- molecular weight (6200- and 27-mers), respectively.  To compare and validate experimental results, a computational model was developed that examined the entropy probability between the different arrangements of two polymer chain lengths of polyacrylic  acid  on  the  surface  of  the  silica  nanosphere.   This  was  done  by  comparing every scenario of surface coverage, using the associated likelihood of its occurrence.  
Our theoretical calculations for the mean-field lattice model also showed preferential surface adsorption of the 450K long chain polymer bases on computing ratio of probabilities as function of the number of long-chain mers adsorbed on the silica colloid.
Both spectroscopic experiment results and the combinatorial model demonstrated that there is an entropic preference for complete adsorption of the longer 450K polyacrylic acid chain  vs  2K. This study provides insights on entropy driven chain-length  dependence of polyelectrolyte adsorption onto surfaces for directing and optimizing their  layer-by-layer self-assembly in organic films.
%, enabling optimization of industrial processes and developments of polyelectrolyte membranes and coatings.  


%Understanding the adsorption behavior of charged self-assembling macromolecules onto surfaces is important in various applications governed by interfacial phenomena, soft matter films fabrication, and biomedical engineering of tissue scaffolds. 
%and stabilization of colloidal suspensions of inorganic particles\cite{Dorris2011}.  
%Most recently, such electrostatic self-assembly adsorption onto small-curvature micro- or nano-particles has received much interest for tissue scaffolds and various other biomedical engineering applications, where the soft, loopy hydrophylic polymer layers have been investigated as ``bio-camouflage'' coatings to improve bio-compatibility at these interfaces\cite{Landry2019}. 
%We study here the role of polyelectrolyte chain length in the competitive adsorption of a simple model polyanion, poly(acrylic acid), onto silica particles capped with a model polycation, poly(allylamine hydrochloride). 
%Theoretical calculations for neutral polymers show that there is preferential adsorption that is dependent on the chain length, number of mers, of the adsorbing polymer.  
%We have performed fluorescence experiments to %confirm this preferential behavior, observing
%observe chain-length dependence between two widely differing molecular weights of fluorescently-labeled PAA polyelectrolytes allowed to adsorb competitively onto an [***IS ''AN'' OKAY HERE?***] oppositely charged PAH polyelectrolytes covering a spherical silica nanoparticle surface in dilute aqueous solution.
%Preferential adsorption was confirmed by comparing the measured fluorescence intensities  of the  two  fluorophore-labeled  (NMA-PAA\textsubscript{450K},  and Dan-PAA\textsubscript{2K}) polyacrylic acid samples presented to the surfaces, representing long or short MW chains of equal mers total, by measuring signalsspecific to the 450K- and 2K-mer chains, respectively. 
%To validate these experimental results, a computational model was developed that examined the entropy probability between the different arrangements of two polymer length chains of polyacrylic  acid  on  the  surface  of  the  silica  nanoparticles.   This  was  done  by  comparing every scenario of surface coverage, using the associated likelihood of its occurrence.  Both spectroscopic experiment results and the combinatorial model demonstrated that there is an entropic preference for complete adsorption of the longer 450K-mers polyacrylic acid chain  vs  2K-mrs. This study provides insights on entropy driven chain-length  dependence  in  layer by layer assembly, enabling optimization of industrial processes and developments of polyelectrolyte membranes and coatings, and rationalizing previous experimental results that suggested an entropic preference for longer chains to adsorb over shorter chains.  
...
% \noindent\textit{
% \begin{enumerate}
%     \item I would put a little bit more about the model, but I don't want to confuse the reader both on the grounds that this is an experimental paper, and because the model is confusing
...
%Growing interest in these self-assembled films can in part be attributed to the fact that their 
...
We used poly(allylamine hydrochloride) (PAH) as received from Polysciences, Inc. Poly(acrylic acid sodium salt) (PAA) of two different molecular weights (M\textsubscript{W} 2K and 450K, corresponding to 28 and 6250 mers respectively) was purchased from Sigma-Aldrich and modified as described in section 2.1.2. 
...
%can deduce that with high probability, 
...
%---more precisely, it is approximately proportional to the fraction of the solution's volume that is occupied by the short polymers (which is a small number).  
...
%Now we turn to the problem of finding $j^*$, the
%value of $j$ that maximizes $|\mathcal{E}_j|$.
%Observe that by Equation (\ref{eq.EEcomp}), we can characterize $j^*$ by saying that $j^*$ is the smallest
%value of $j$ for which $|\mathcal{E}_{j+1}|/|\mathcal{E}_j|$ is less than 1.
%Thus our strategy will be to approximate the ratio
%$|\mathcal{E}_{j+1}|/|\mathcal{E}_j|$ and to identify
%the value of $j$ for which it equals 1 --- i.e., where
%the ratio changes from being greater than 1 to being less than 1.
...
%the right-hand side of Equation (\ref{eq.Yratio2}) 
$h(j)$ is less than 1. 
...
%\subsubsection{4.2.1 
Validation: Model Comparison to Experimental Results}    %4.2
    \label{sec-res-theor}

%To verify the mean field lattice model, we test it with the data from the experiment. 
To validate our experimental results, 
we compare the data to the numerical predictions of our mean field lattice model.
The following table gives the numerical values that we use for the key parameters in our model.

\begin{center}
\begin{tabular}{|c|c|c|}
    \hline
     Variable & Description  & Value   \\ \hline\hline
    $\leng_L$ &  mers in a long chain &  6200 \\  
    $\leng_S$ & mers in a short chain &27  \\
    $n$ & $\leng_L/\leng_S$  &   229  \\
    $n_L$ & number of $\leng_L$-mers & 206  \\
    $n_S$  & number of $\leng_S$-mers & 5562  \\
    $q$   & surface coordination number & 4  \\
    $V_{latt}$  & sites in model volume & $5.3\times          10^{10}$ (????)  \\
    $S_{latt}$  & sites on surface of sphere  & 142600 (???)  \\
...
$\left(\frac{|\mathcal{E}_{j+1}|}{|\mathcal{E}_j|}\right)$ as a function of the number $j$ of $\leng_L$-mers (long-chains) adsorbed to the colloid surface. We observe that the ratio of probabilities %approaching one, from functional values 
are all much
...
%[*** NM:  WHAT POINT IS THIS FINAL PARAGRAPH TRYING TO MAKE?  IT DOESN'T SEEM TO BE SAYING ANYTHING NEW.  DO WE NEED IT? OR CAN IT BE MADE MORE CONCISE?  ****]

To summarize, our model calculations show that the ratio of probabilities $|\mathcal{E}_{j+1}|/|\mathcal{E}_j|$,
%*****or whatever ends up the y axis in the figure***, 
...
%the one means that the values will never get any higher than that and thus it is a maximum (probability of occurring in relation to the rest), the location (on the j line , ie; number of longs on surface) of this maximum is key to seeing if our hypothesis is correct, that the maximum is complete coverage by longs 
