...
It will be convenient to let $n=\leng_L/\leng_S$, 
so that
...
\end{equation}
 ...
 the number of $\leng_S$-mers (short-chains) adsorbed on the surface must be $n(a-j)$ [because there are $S_{latt}=an\leng_S$ sites on the surface, all occupied, of which $j\leng_L$ are occupied by $\leng_L$-mers (long-chains), leaving $S_{latt}-j\leng_L\,=\, an\leng_S-jn\leng_S$ to be  occupied by $\leng_S$-mers (short-chains)]; 
...
***EDIT*** 
Let $j^*$ be the smallest value of $j$ for which the right-hand side of Equation (\ref{eq.Yratio2}) is 
    ...
    1  \;\approx \; 
     ...
     %\frac{ V_{latt}A_3W^{\gamma_3-1}(q-1)^2}{aWq} \,
          \frac{a-j^*-\frac{1}{2}}{n_S-n(a-j^*-\frac{1}{2})}    \,,  
             ...
             %K \;=\;   \frac{ V_{latt} }{aW}\, \frac{   A_3W^{\gamma_3-1}(q-1)^2}{q} \,,
\end{equation}
As explained in the Supplement, this leads to the approximation
\begin{equation}
    \label{eq.jstar}
     1-\frac{j^*}{a}     \; \approx   \; 
