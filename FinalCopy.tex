...
We now wish to estimate the size of each $\mathcal{E}_j$ given that this will give us a clear idea of preferential configurations of the experimental system.The configuration of a single  polymer in a dilute solution is often modeled by a self-avoiding walk (SAW) in a lattice, i.e.\  a path in the lattice that does not visit any site more than once. A fundamental property of SAWs is the following.
Let $c_{\ell}$ be
