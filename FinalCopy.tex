...
%Professor Madras informed me that section number isn't a standard for macromolecule
...
Here we develop a combinatorial approximation method that outputs the likelihood of each possible configuration, with which we model the preferential behaviour of the system.
...
fully discrete model to analyze this phenomenon.  We will also maintain the assumption from the experiment, that the enthalpic energy is identical in both the scenarios of either complete short or long-chain adsorption to the colloid surface. 
...
We shall often refer to our short and long-chain polymers as $W$-mers and $nW$-mers respectively.
...
%Recall that our ensemble consists of $n_S$ (short) $W$-mers (short-chains) and $n_L$ (long) $nW$-mers (long-chains).
%Of these, some are adsorbed onto the surface of the sphere, and the others float freely in the 
%solution of volume $V$.  We write $j$ for the number of (long) $nW$-mers (long-chains) that are adsorbed 
...
% \begin{verse}
%  ($i$) the number of $nW$-mers (long-chains) in solution is $n_L-j$;
%  \\
%  ($ii$) the number of $W$-mers (short-chains) adsorbed on the surface is $n(a-j)$ (because there are $anW$ sites on the
%  surface, all occupied, of which $jnW$ are occupied by $nW$-mers (long-chains), leaving $anW-jnW$ to be 
%  occupied by $W$-mers (short-chains)); and
%  \\
%  ($iii$) the number of $W$-mers (short-chains) in solution is $n_S-n(a-j)$.
%  \end{verse}  
We shall write $\mathcal{E}_j$ for the set of all configurations that have exactly $j$ $nW$-mers (long-chains) adsorbed 
...
% \]
Thus, to find the most likely number of $nW$-mers (long-chains) %(and $W$-mers (short-chains))
to be adsorbed onto the sphere,
...
Let the number of SAWs that start from a specified lattice site (``the origin'') and visit a total of $\ell$ sites be represented as $c_{\ell}$. 

%%KILLED THIS AND PUT ORIGINAL BACK
%Additionally, let $\mu_3$ and $A_3$ be constants that depend on the choice of lattice, and $\gamma_3$ be a universal critical exponent that is the same for all three-dimensional lattices, their values are known to be approximately, \cite{Chen2002,Madras2013} 
...
% For a particular choice of $j$, we want to know the number of ways to place $j$ $nW$-mers (long-chains) 
% and $a(n-j)$ $W$-mers (short-chains) on the surface, without overlapping, to fill all $anW$ lattice sites on the surface.
...
% $(k-1)$ $nW$-mers (long-chains) have already been placed.  
...
% ${\ell}-1$ $W$-mers (short-chains) (or a total of $(\ell-1)W$ monomers) have already been placed.   The same 
...
% polymers.   
% For $\mathcal{E}_j$, we have $j$ adsorbed $nW$-mers (long-chains), $na-nj$ adsorbed $W$-mers (short-chains), 
% $n_L-j$ desorbed $nW$-mers (long-chains), and $n_S-n(a-j)$ desorbed $W$-mers (short-chains).   Thus, recalling 
